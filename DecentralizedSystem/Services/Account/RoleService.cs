...
namespace DecentralizedSystem.Services.Account
{
    public interface IRoleService
    {
        ...
        Task<List<RoleModel>> GetListRoleAsync(string userName, string password, string appid);
    }
    public class RoleService : IRoleService
    {
        ...
        public async Task<List<RoleModel>> GetListRoleAsync(string userName, string password, string appid)
        {
            var url = _config["App:UrlAuth"];
            ...
            var response = await restClient.ExecuteAsync(request);
            return response.Content.DeserializeObject<Dictionary<string, object>>()["DATA_MENU"]
                                   .MapProp<List<Dictionary<string, object>>>()
                                   .Select(s => new RoleModel
                                   {
                                       Id = s["CODE"].ToString(),
                                       Name = s["ROLE_GROUP_NAME"].ToString(),
                                   })
                                   .ToList();
        }
        ...
        public async Task<UserModel> GetUserInfoAsync(string userName, string password)
        {
            ...
            if (dataLogin["STATUS"].ToString() == "1")
            {
                ...
                var brachInfo = dataLogin["BRANCH_INFO"].MapProp<List<Dictionary<string, object>>>().First();
                var branchCode = brachInfo["BCODE_FCC"].ToString();
                ...
                return new UserModel
                {
                    ...
                    Branch = new BranchModel
                    {
                        Id = branchCode,
                        Code = branchCode,
                        Name = brachInfo["BNAME"].ToString(),
                        Address = brachInfo["BADDRESS"].ToString(),
                    },
                    Department = new DepartmentModel
                    {
                        Id = userInfo["DEPID"]?.ToString() ?? "",
                        Code = branchCode,
                        Name = userInfo["DEP_NAME"]?.ToString() ?? "",
                    }
                };
            }
            return null;
